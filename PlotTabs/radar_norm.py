...
def radar_norm_tab(df_means,df_stdev, Time,Treatments,number_cmpds_run,Groups):
    ...
    df_temp[Groups]=norm_fac*df_temp[Groups] 
    ...
    for tt, tt_str in enumerate(time_vals):
        if len(Treatments) == 4:
            df_for_radar=df_temp.loc[(df_temp[Time[0]] == tt_str) & (df_temp[Treatments[0]]==tm0_vals[0]) & (df_temp[Treatments[1]]==tm1_vals[0]) & (df_temp[Treatments[2]]==tm2_vals[0]) & (df_temp[Treatments[3]]==tm3_vals[0])]
        elif len(Treatments) == 3: 
            df_for_radar=df_temp.loc[(df_temp[Time[0]] == tt_str) & (df_temp[Treatments[0]]==tm0_vals[0]) & (df_temp[Treatments[1]]==tm1_vals[0]) & (df_temp[Treatments[2]]==tm2_vals[0]) ]        
        elif len(Treatments) == 2: 
            df_for_radar=df_temp.loc[(df_temp[Time[0]] == tt_str) & (df_temp[Treatments[0]]==tm0_vals[0]) & (df_temp[Treatments[1]]==tm1_vals[0]) ]
        elif len(Treatments) == 1: 
            df_for_radar=df_temp.loc[(df_temp[Time[0]] == tt_str) & (df_temp[Treatments[0]]==tm0_vals[0]) ]
        df_radar_values=df_for_radar[def_grps]
        radar_vals=list(df_radar_values.values.flatten())
        flist.append(radar_vals)
    x_label,y_label =  circ(100,theta_label)
    x_vals,y_vals=circ(100)
    ...
    def update_data(attrname, old, new):
        ...
        df_temp=df_means
        norm_fac=100/np.max(df_means[compounds_in_radar].max())
        df_temp[compounds_in_radar]=norm_fac*df_temp[compounds_in_radar] 
        theta_label = np.linspace(0, 2*np.pi, len(compounds_in_radar), endpoint=False)
        theta_label += np.pi/2 # rotate theta such that the first axis is at the top
        flist=[]
        for tt, tt_str in enumerate(time_vals): 
            if len(Treatments) == 4:
                df_for_radar=df_temp.loc[(df_temp[Time[0]] == tt_str) & (df_temp[Treatments[0]]==select0.value) & (df_temp[Treatments[1]]==select1.value) & (df_temp[Treatments[2]]==select2.value) & (df_temp[Treatments[3]]==select3.value)]
            elif len(Treatments) == 3: 
                df_for_radar=df_temp.loc[(df_temp[Time[0]] == tt_str) & (df_temp[Treatments[0]]==select0.value) & (df_temp[Treatments[1]]==select1.value) & (df_temp[Treatments[2]]==select2.value) ]        
            elif len(Treatments) == 2: 
                df_for_radar=df_temp.loc[(df_temp[Time[0]] == tt_str) & (df_temp[Treatments[0]]==select0.value) & (df_temp[Treatments[1]]==select1.value) ]
            elif len(Treatments) == 1: 
                df_for_radar=df_temp.loc[(df_temp[Time[0]] == tt_str) & (df_temp[Treatments[0]]==select0.value) ]
    ...
    tab = Panel(child=layout,title='Norm Radar Chart')
